...
function buildReviseChips(opts, type) {
  return opts.map(o => {
    const hold = S.reviseWordAction
      ...
      : '';
    return '<button class="chip ' + type + '-chip revise-chip" data-correct="' + o.correct + '" onclick="reviseClick(this)"' + hold + '>' + esc(o.word) + '</button>';
  }).join('');
}
...
function btn(attr, key, p) {
  // helper to check which button matches current value
...
function tog(id, key) {
  document.getElementById(id).checked = !!pending[key];
}
