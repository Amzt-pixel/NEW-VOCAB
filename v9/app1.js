...
let S = {
  ...
  mcqOptions: 4, mcqMultipleCorrect: false, mcqMaxCorrect: 1, mcqRandomize: false,
...
function showStudy(word, entry) {
  ...
  // Hide MCQ card
  document.getElementById('mcqCard').classList.add('hidden');
}
...
function showMCQ(word, entry) {
  ...
  // Meaning tab — read style only if mcqShowMeaning ON
  ...
  setCards(syns, ants, showDef, simSyms, simAnts);
  reorderTabs();
  activateFirstTab([...syns, ...simSyms], [...ants, ...simAnts], hasDef, showDef);

  // Build MCQ options
  buildMCQCard(word, entry);
}

function buildMCQCard(word, entry) {
  const mcqEl = document.getElementById('mcqCard');
  if (!entry?.definition) { mcqEl.classList.add('hidden'); return; }

  const total = S.mcqOptions; // 4, 5, or 6

  // Determine correct count
  let correctCount = 1;
  if (S.mcqMultipleCorrect) {
    correctCount = S.mcqRandomize
      ? Math.floor(Math.random() * S.mcqMaxCorrect) + 1
      : S.mcqMaxCorrect;
  }

  // Gather correct definitions — current word + similar-id words (same positive id = exact syns)
  const currentEntry = csvData.find(r => r.word === word);
  const correctDefs = [];
  if (currentEntry?.definition) correctDefs.push(currentEntry.definition);
  if (S.mcqMultipleCorrect && correctCount > 1) {
    // Pull definitions from exact synonyms (same id) as additional correct options
    const synEntries = csvData.filter(r => r.id === currentEntry.id && r.word !== word && r.definition);
    shuffle(synEntries).slice(0, correctCount - 1).forEach(r => correctDefs.push(r.definition));
  }
  const pickedCorrect = correctDefs.slice(0, correctCount);

  // Distractor pool — other words in studyList with definitions, exclude current word & its syns
  const excludeWords = new Set([word, ...getSyns(word)]);
  const distractorPool = studyList
    .filter(w => !excludeWords.has(w))
    .map(w => csvData.find(r => r.word === w))
    .filter(r => r?.definition)
    .map(r => r.definition)
    .filter(d => !pickedCorrect.includes(d));

  const fillers = shuffle([...new Set(distractorPool)]).slice(0, total - pickedCorrect.length);
  const options = shuffle([
    ...pickedCorrect.map(d => ({ def: d, correct: true })),
    ...fillers.map(d => ({ def: d, correct: false }))
  ]);

  const hint = S.mcqMultipleCorrect
    ? '<div class="mcq-hint">Select ' + pickedCorrect.length + ' correct answer' + (pickedCorrect.length > 1 ? 's' : '') + '</div>'
    : '<div class="mcq-hint">Select the correct definition</div>';

  const optionsHTML = options.map((o, i) =>
    '<button class="mcq-option" data-correct="' + o.correct + '" data-index="' + i + '" onclick="mcqClick(this)">'
    + '<span class="mcq-option-letter">' + String.fromCharCode(65 + i) + '</span>'
    + '<span class="mcq-option-text">' + esc(o.def) + '</span>'
    + '</button>'
  ).join('');

  mcqEl.innerHTML = hint + '<div class="mcq-options">' + optionsHTML + '</div>';
  mcqEl.classList.remove('hidden');
}

function mcqClick(btn) {
  if (btn.classList.contains('mcq-correct') || btn.classList.contains('mcq-incorrect')) return;
  const ok = btn.dataset.correct === 'true';
  btn.classList.add(ok ? 'mcq-correct' : 'mcq-incorrect');

  // In single correct mode, reveal all after any click
  if (!S.mcqMultipleCorrect) {
    const card = btn.closest('#mcqCard');
    card.querySelectorAll('.mcq-option').forEach(opt => {
      if (opt === btn) return;
      if (opt.dataset.correct === 'true') opt.classList.add('mcq-revealed');
      else opt.classList.add('mcq-disabled');
    });
  }
}
...
function syncSettingsUI() {
  ...
  tog('mcqRandomizeToggle','mcqRandomize'); tog('mcqWordActionToggle','mcqWordAction');
...
function bindSettingsEvents() {
  ...
  bindTog('mcqRandomizeToggle','mcqRandomize'); bindTog('mcqWordActionToggle','mcqWordAction');
  bindTog('mcqShowMeaningToggle','mcqShowMeaning');
  bindTog('mcqShowSimilarToggle','mcqShowSimilar');
  ...
  document.getElementById('settingsSave').addEventListener('click', () => {
    if (pending) {
      ...
      const mcqChanged = S.mode === 'mcq' && (
        ...
        pending.mcqMaxCorrect !== S.mcqMaxCorrect || pending.mcqRandomize !== S.mcqRandomize ||
        pending.mcqShowMeaning !== S.mcqShowMeaning || pending.mcqShowSimilar !== S.mcqShowSimilar
      );
