...
function setCards(syns, ants, showDef, simSyns, simAnts, hasBengali) {
  simSyns    = simSyns    || [];
  simAnts    = simAnts    || [];
...
function showStudy(word, entry) {
  ...
  // Bengali section
  const hasBengali = S.showTranslation && !!entry?.bengaliDef;
  if (hasBengali) {
    let html = '<div class="bengali-def">' + esc(entry.bengaliDef) + '</div>';
    if (entry.bengaliEx1) html += '<div class="bengali-example">' + esc(entry.bengaliEx1) + '</div>';
    if (entry.bengaliEx2) html += '<div class="bengali-example">' + esc(entry.bengaliEx2) + '</div>';
    if (entry.bengaliEx3) html += '<div class="bengali-example">' + esc(entry.bengaliEx3) + '</div>';
    document.getElementById('bengaliContent').innerHTML = html;
  }

  setCards(syns, ants, hasDef, simSyns, simAnts, hasBengali);
  reorderTabs();
