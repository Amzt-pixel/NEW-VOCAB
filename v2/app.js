...
function openAddTo(word, e) {
  ...
  // Update hide button label based on current state
  const isHidden = hiddenWords.has(word);
  document.getElementById('addToHideBtn').firstChild.textContent = isHidden ? '👁️' : '🚫';
  document.getElementById('addToHideLabel').textContent = isHidden ? 'Unhide' : 'Hide';
  document.getElementById('addToHideBtn').classList.toggle('active', isHidden);
  ...
  document.querySelectorAll('.addto-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (flag === 'queue') {
        ...
        if (!already) { customQueue.push({ word }); btn.classList.add('active'); }
      ...
      // bookmark/important/learned/favorite handled in Phase 9
    });
  });
