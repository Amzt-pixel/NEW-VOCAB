  ...
  if (panelTab === 'list') {
    ...
    container.innerHTML = items.map(({ w, i }) => {
      ...
      return `<div class="panel-item${isCurrent ? ' current' : ''}${isHidden ? ' word-hidden' : ''}"
          ...
          ontouchmove="cancelPanelHoldOnTouch(event)"
...
// Panel hold — scroll cancel + left-zone navigation
...
function cancelPanelHold() {
  // Called on touchmove — finger is scrolling, cancel everything
  clearPanelHold();
  panelWasHold = false;
}

function cancelPanelHoldOnTouch(e) {
...
function cancelPanelHoldOnMove(e) {
  // Called on mousemove — only cancel if mouse has moved significantly
