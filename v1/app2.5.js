...
function updateWordListPanel() {
  ...
  if (panelTab === 'list') {
    ...
    // Touch hold via event delegation on the container — survives innerHTML re-renders.
    // We track the target panel-item and use a movement threshold to allow scrolling.
    attachPanelTouchDelegation(container);
...
// ══════════════════════════════════════
// PANEL TOUCH DELEGATION
// Attached once to #wordListContent — survives innerHTML re-renders.
// Uses a movement threshold so scrolling still works normally.
// ══════════════════════════════════════
let panelTouchDelegationAttached = false;

function attachPanelTouchDelegation(container) {
  if (panelTouchDelegationAttached) return;
  panelTouchDelegationAttached = true;
  ...
  container.addEventListener('touchend',    clearPanelHold, { passive: true });
  container.addEventListener('touchcancel', clearPanelHold, { passive: true });
}
