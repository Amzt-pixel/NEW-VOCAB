...
function renderSearchResults(query, containerId, onSelect) {
  ...
  const selectFn = onSelect || (w => jumpToWordFromSearch(w));
  ...
  function group(title, list) {
    ...
    return `<div>
      ...
      ${list.slice(0,20).map(w => {
        ...
        return `<div class="search-item" onclick="(${selectFn.toString()})('${escapeHtml(w)}')">
          <div>
  ...
  if (panelTab === 'list') {
    ...
    container.innerHTML = items.map(({ w, i }) => {
      ...
      return `<div class="panel-item${isCurrent ? ' current' : ''}${isHidden ? ' word-hidden' : ''}"
          ...
          onmouseup="handlePanelClick(event, this)"
          onmouseleave="clearPanelHold()"
          ontouchstart="startPanelHold(event, this)"
          ontouchend="handlePanelClick(event, this)"
          ontouchcancel="clearPanelHold()">
        <span class="panel-item-num">${i+1}</span>
        <span class="panel-item-word">${escapeHtml(w)}${isHidden ? ' 🚫' : ''}</span>
        <div class="panel-item-actions">
          <button class="panel-action-btn panel-action-add" onclick="openAddTo('${escapeHtml(w)}', event)" title="Add to…">➕</button>
          <button class="panel-action-btn ${isHidden ? 'panel-action-unhide' : 'panel-action-hide'}"
            onclick="toggleHideWord('${escapeHtml(w)}', event)"
            title="${isHidden ? 'Unhide' : 'Hide from session'}">
            ${isHidden ? '👁️' : '🚫'}
          </button>
        </div>
      </div>`;
    ...
    container.innerHTML = items.map(({ word }, i) =>
      `<div class="panel-item" onclick="startSessionAtWord('${escapeHtml(word)}')">
        ...
        <button class="panel-action-btn panel-action-hide" onclick="removeFromQueue('${escapeHtml(word)}', event)" title="Remove">✕</button>
      </div>`
...
// Change 2: press-hold on panel item to reveal actions
...
function handlePanelClick(e, el) {
  if (panelHoldTimer) {
    // Short tap — navigate to word
    clearPanelHold();
...
function applySettings() { applyAccent(); applyFontSize(); }
...
function bindEvents() {
  ...
  // Change 8: show all root words
  document.getElementById('showAllWordsBtn').addEventListener('click', () => {
    renderHomeWordList(!homeListExpanded);
  });
