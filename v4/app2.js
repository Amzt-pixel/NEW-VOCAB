...
function bindSettingsEvents() {
  ...
  // Nav filter checkboxes — at least one must always be checked (default fallback: Syn/Ant)
...
function closeModal(id) { document.getElementById(id).classList.add('hidden'); }
  ...
  document.querySelectorAll('[data-qstep]').forEach(b => b.addEventListener('click', () => {
    ...
    b.classList.add('active');
    pending.stepNumber = parseInt(b.dataset.qstep);
  }));
  ...
  document.querySelectorAll('[data-qdelta]').forEach(b => b.addEventListener('click', () => {
    ...
    b.classList.add('active');
    pending.navDelta = parseInt(b.dataset.qdelta);
  }));
  ...
  // Nav filter checkboxes — at least one mandatory
  ['qNavSynAntChk','qNavDefinedChk','qNavRootwiseChk'].forEach(id => {
    document.getElementById(id).addEventListener('change', e => {
      ...
      if (!e.target.checked) {
        const others = (id !== 'qNavSynAntChk' && pending.navSynAnt)
                    || (id !== 'qNavDefinedChk' && pending.navDefined)
                    || (id !== 'qNavRootwiseChk' && pending.navRootwise);
...
function bindAll() {
  ...
  // Quick Nav popup
  document.getElementById('quickNavClose').addEventListener('click', () => { pending = null; closeModal('quickNavOverlay'); });
  ...
  document.getElementById('quickNavOverlay').addEventListener('click', e => { if (e.target.id === 'quickNavOverlay') { pending = null; closeModal('quickNavOverlay'); } });
  navBindQuickNav();
