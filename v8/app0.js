...
function goHome() {
  if (sessionLive) {
    // Sync dropdowns to current session state
    document.getElementById('categorySelect').value = S.category || '';
    document.getElementById('orderSelect').value    = S.orderMode;
    // Clear demo, freeze to static
    demoStudyList  = [];
...
function goStudy() {
  // Clear demo when entering study screen
  demoStudyList  = [];
...
function updateStats() {
  const source = demoStudyList.length ? demoStudyList
               : studyList.length     ? studyList
               : null;
  ...
  const cats      = new Set(source.map(w => {
...
function renderHomeList(n) {
  ...
  if (btn1) btn1.title = homeListMode === 'root' ? 'Switch to All' : 'Switch to Root';
  if (btn2) {
  ...
  let html = visible.map((w, i) => {
    ...
    const e   = csvData.find(r => r.word === w);
    return '<div class="word-list-item" onclick="jumpToWord(\'' + esc(w) + '\')">'
      + '<span class="word-list-num">' + (i+1) + '</span>'
      + '<span>' + esc(w) + '<span class="word-list-id"> (#' + (e?.id || '—') + ')</span></span>'
...
function startSession() {
  ...
  S.category  = document.getElementById('categorySelect').value;
...
function quitSession() {
  ...
  document.getElementById('orderSelect').value    = 'az';
  demoStudyList  = buildDemoStudyList();
